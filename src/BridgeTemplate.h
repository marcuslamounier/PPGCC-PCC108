/**
 * @file BridgeTemplate.h
 *
 * @brief Handle/body idiom template
 * 
 * The classes Handle and Body implements "bridge" design pattern (also known as "handle/body idiom").The class Body was implemented based on the class teCounted writed by Ricardo Cartaxo and Gilberto Camara and founded in the geographic library TerraLib.
 *
...
// extern int numHandleCreated; //Only for testing
// extern int numHandleDeleted; //Only for testing
// extern int numBodyCreated; //Only for testing
// extern int numBodyDeleted; //Only for testing
...
public:	
	...
	/// constructor
	Handle<T>( ){
		// numHandleCreated++; //Only for testing
		pImpl_ = new T; 
        pImpl_->attach();  
	}
	...
	virtual ~Handle<T>() {
		pImpl_->detach();
		// numHandleDeleted++; //Only for testing
	}
...
public:
	...
	void attach ()	{
		refCount_++;
		// numBodyCreated++; //Only for testing
	}
